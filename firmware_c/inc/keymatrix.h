...
const uint8_t keymatrix_bootmode_normal[8][9]={//outer index corresponds to column (therefore move one row up or down below), the inner index is the row (therefore move right left in the list below) according to layout see above
    {KEY_ESCAPE ,KEY_TAB    ,KEY_CAPSLOCK   ,KEY_LEFTSHIFT      ,KEY_LEFTCTRL       ,KEY_8      ,KEY_I              ,KEY_K          ,KEY_COMMA          },
    {KEY_1      ,KEY_Q      ,KEY_A          ,KEY_Z              ,KEY_LEFTOS         ,KEY_9      ,KEY_O              ,KEY_L          ,KEY_POINT          },
    {KEY_2      ,KEY_W      ,KEY_S          ,KEY_X              ,KEY_LEFTALT        ,KEY_0      ,KEY_P              ,KEY_SEMIKOLON  ,KEY_SLASH          },
    {KEY_3      ,KEY_E      ,KEY_D          ,KEY_C              ,KEY_SPACEBAR       ,KEY_MINUS  ,KEY_LEFTBRACE      ,KEY_APOSTROPHE ,KEY_RIGHTSHIFT     },
    {KEY_4      ,KEY_R      ,KEY_F          ,KEY_V              ,KEY_RIGHTALT       ,KEY_EQUAL  ,KEY_RIGHTBRACE     ,KEY_RETURN     ,KEY_PAGEUP         },
    {KEY_5      ,KEY_T      ,KEY_G          ,KEY_B              ,KEY_RIGHTOS        ,KEY_DELETE ,KEY_BACKSLASH      ,KEY_LEFTARROW  ,KEY_PAGEDOWN       },
    {KEY_6      ,KEY_Y      ,KEY_H          ,KEY_N              ,KEY_APPLICATION    ,KEY_INSERT ,KEY_DELETEFORWARD  ,KEY_DOWNARROW  ,KEY_UPARROW        },
    {KEY_7      ,KEY_U      ,KEY_J          ,KEY_M              ,KEY_RIGHTCTRL      ,KEY_HOME   ,KEY_END            ,KEY_RIGHTARROW ,KEY_BACKSLASHNONUS }
};
...
void reportPressedKeys(uint32_t protocol,keydata KeyDataListIn[8][9],uint8_t* report_out){
    if(protocol==boot_protocol){
        uint32_t keycodebyte=2; //Start at the third byte with the keycodes because [0] are modifiers and [1] is reserved
        ...
        while((currentrow<9)&&(keycodebyte<8)){     //break if we have finished or if we have more than 6 key rollover
            ...
            if(keycode==KEY_LEFTCTRL){
                report_out[0] |=KEY_MOD_LEFTCTRL;
            }else if(keycode==KEY_LEFTSHIFT){
                report_out[0] |=KEY_MOD_LEFTSHIFT;
            }else if(keycode==KEY_LEFTALT){
                report_out[0] |=KEY_MOD_LEFTALT;
            }else if(keycode==KEY_LEFTOS){
                report_out[0] |=KEY_MOD_LEFTOS;
            }else if(keycode==KEY_RIGHTCTRL){
                report_out[0] |=KEY_MOD_RIGHTCTRL;
            }else if(keycode==KEY_RIGHTSHIFT){
                report_out[0] |=KEY_MOD_RIGHTSHIFT;
            }else if(keycode==KEY_RIGHTALT){
                report_out[0] |=KEY_MOD_RIGHTALT;
            }else if(keycode==KEY_RIGHTOS){
                report_out[0] |=KEY_MOD_RIGHTOS;
